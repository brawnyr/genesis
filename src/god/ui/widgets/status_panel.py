...
class StatusPanel(Widget):
    ...
    def compose(self) -> ComposeResult:
        yield Static(Text(""), id="loop-bar")
        yield Static(Text(""), id="pass-counter")
        yield Static(Text(""), id="metronome-status")
    ...
    def update_loop(self, progress: float, current_bar: int, total_bars: int) -> None:
        ...
        self.query_one("#loop-bar", Static).update(Text(f" BAR {current_bar}/{total_bars}  {bar} "))
    ...
    def update_passes(self, count: int) -> None:
        ...
        self.query_one("#pass-counter", Static).update(Text(f" PASSES  {dots} "))

    def update_metronome(self, enabled: bool, sound_name: str) -> None:
        state = "ON" if enabled else "OFF"
        self.query_one("#metronome-status", Static).update(Text(f" METRONOME {state}  {sound_name} "))
