...
private let logger = Logger(subsystem: "genesis", category: "PadBank")
    ...
    var config: PadConfig {
        ...
        for pad in pads {
            if let path = pad.samplePath {
                cfg.assignments[String(pad.index)] = PadAssignment(path: path, name: pad.name, choke: pad.choke)
            }
        }
        return cfg
    }
    ...
    mutating func loadConfig() throws {
        ...
        for (key, assignment) in cfg.assignments {
            ...
            do {
                ...
                pads[index].choke = assignment.choke ?? true
            } catch {
